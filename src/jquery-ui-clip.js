;(function($){
	...
	// Wenn das Fenster wechselt, wird die Animation angehalten.
	// TODO Aus irgendeinem Grund läuft sie noch nicht weiter, wenn man wieder zu dem aktuellen Fenster springt O.o
	$(window)
		.bind('focus', function(){
			$.timer.enable();
		})
	...
	$.widget("ui.clip", {
		...
		_changeView: function(frame){
			if(!this.element.parent().is('.ui-clip-wrapper')){
				...
				.wrap($(document.createElement('span')).addClass('ui-clip-wrapper')
						.css({
							position: "relative",
							display: "block",
